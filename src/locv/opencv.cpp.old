...
static int fromGDCM(lua_State *L) {
    ...
    const unsigned int type = getfield(L, "cvtype");
...
// preferable to send a Rect; instead of Mat; only need for size
...
static int properties(lua_State *L) {
    cv::Mat *m = checkmat(L, 1);
    if (m->cols > 1)
	luaL_error(L, "Only contour objects.");
...
static int warpAffine(lua_State *L) {
   cv::Mat *m = checkmat(L, 1);
   cv::Mat *matrix = checkmat(L, 2);
   cv::Mat dst;
   cv::warpAffine(*m, dst, *matrix, m->size());
   cv::Mat **um = newmat(L);
   *um = new cv::Mat(dst);
   return 1;
}
...
//	    alpha = luaL_checknumber(L, 3);
    ...
    cv::mixChannels(input, 3, &rgb, 1, from_to, 3);
...
// can be changes to return different results for each channel
...
static int convexHull(lua_State *L) {
    cv::Mat *c = checkmat(L, 1);
    if (c->cols >1)
	luaL_error(L, "Contour object required.");
    cv::Mat hull;
...
/*
static int rgb2hsl(lua_State *L) {
    cv::Mat *m = checkmat(L, 1);
    check83(L, m);
    cv::Mat dst;
    cv::cvtColor(*m, dst, CV_RGB2HSL);
    *um = new cv::Mat(dst);
    return 1;
}
*/
...
static int hideImage(lua_State *L) {
...
static int mat2str(lua_State *L) {
    ...
    int rows = m->rows;
    int cols = m->cols;
    lua_pushfstring(L, "ocvImage{ rows = %d, cols = %d, depth = '%s', channels = %d }", rows, cols, depth2str(m->depth()), m->channels());
    return 1;
}
